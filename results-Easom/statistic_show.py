...
input_dir = "/home/ziangchen9/home/ziangchen9/home/ziangchen9/yao-SDSC6002/Experiments/results-Easom/processed_data"
...
def plot_grouped_data(group_name, group_data, color_palette):
    ...
    # Find maximum length among all datasets in this group
    max_length = max(len(data.columns) for data in group_data.values())
    x_values = np.arange(max_length)

    sample_freq = max(1, max_length // 15)  # Show ~15 error bars per line

    # Stagger control
    base_stagger = 1.2  # Moderate stagger
    ...
    for idx, (filename, data) in enumerate(group_data.items()):
        ...
        std_dev = data.std(axis=0).values
        n = len(data)
        ...
        if len(mean_values) < max_length:
            pad_length = max_length - len(mean_values)
            mean_values = np.pad(mean_values, (0, pad_length), constant_values=np.nan)
            ci_width = np.pad(ci_width, (0, pad_length), constant_values=np.nan)
        ...
        # Plot main line (only for valid data points)
        valid_mask = ~np.isnan(mean_values)
        ax.plot(
            staggered_x[valid_mask],
        ...
        error_x = staggered_x[valid_mask][::sample_freq]
        error_y = mean_values[valid_mask][::sample_freq]
        error_yerr = ci_width[valid_mask][::sample_freq]
