...
def extract_group_name(filename):
    """Extract group name from filename (content before second '+')"""
    clean_name = filename.replace("result_reals_", "").replace("_processed", "")
    parts = clean_name.split("+")

    if len(parts) >= 2:
        return "+".join(parts[:2]).strip("_")
    return "default_group"


def clean_algorithm_name(filename):
    """Clean algorithm name by keeping content after second '+'"""
...
def plot_grouped_data(group_name, group_data, color_palette):
    ...
    # Stagger control
    base_stagger = -1.0  # Moderate stagger
    ...
    for idx, (filename, data) in enumerate(group_data.items()):
        ...
        if len(staggered_x) != len(mean_values):
            print(
                f"Error in file {filename}: Length mismatch between staggered_x ({len(staggered_x)}) and mean_values ({len(mean_values)})"
            )
            continue
    ...
    # Reference line
    ax.axhline(y=-1, color="black", linestyle="--", linewidth=1.2, alpha=0.6, zorder=5)
...
def main():
    ...
    # Group files by content before second '+'
    file_groups = defaultdict(dict)

    for filename in csv_files:
        try:
            print(f"Processing file: {filename}")  # Checkpoint: Print filename
