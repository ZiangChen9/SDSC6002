...
def plot_grouped_data(group_name, group_data, color_palette):
    ...
    for idx, (filename, data) in enumerate(group_data.items()):
        ...
        # Truncate to the first 11 data points
        max_points = 11
        mean_values = mean_values[:max_points]
        std_dev = std_dev[:max_points]
        ci_width = ci_width[:max_points]
        x_values = np.arange(len(mean_values))  # Update x_values to match truncated length
        ...
        if len(staggered_x) != len(mean_values):
            print(f"Error in file {filename}: Length mismatch between staggered_x ({len(staggered_x)}) and mean_values ({len(mean_values)})")
            continue
