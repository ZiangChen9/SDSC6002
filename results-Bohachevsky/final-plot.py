...
def plot_all_data(all_files_data, color_palette):
    ...
    # Ensure all files have at least 11 columns
    min_columns = 11
    ...
    for idx, (filename, data) in enumerate(all_files_data.items()):
        # Truncate data to the first 11 columns
    ...
    # Styling
    ax.set_title(f"Performance Comparison: Bohachevsky", fontweight='bold', pad=15)
...
def main():
    ...
    # Read all files and prepare data for plotting, limit to first 11 files
    all_files_data = {}
    max_files = 11
    file_count = 0
    ...
    for filename in csv_files:
        ...
        try:
            ...
            # Check if the file has at least 11 columns
            if len(data.columns) >= 11:
                all_files_data[filename] = data
                file_count += 1
            else:
                print(f"Warning: File {filename} has fewer than 11 columns. Skipping.")
    ...
    if all_files_data:
        ...
        print("No valid files with at least 11 columns found.")
